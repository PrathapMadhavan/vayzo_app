...
import {
  ArrowLeft, MessageSquare, ShieldBan, ShieldCheck,
  Phone, Mail, Calendar, MapPin, User, Wallet,
  FileText, AlertTriangle, History, ChevronRight,
  TrendingUp, Package, CreditCard, Check, X
} from 'lucide-react';
import {
  getCustomerById, updateCustomerStatus,
  getCustomerWallet, getCustomerTransactions,
  getCustomerRequests, getCustomerComplaints
} from '../api/usersApi';
...
const InfoRow = ({ label, value }) => (
  <div className="flex items-start py-2.5 border-b border-border/40 last:border-0 gap-2">
...
const StatBox = ({ label, value, sub, color = 'primary' }) => {
  ...
  return (
    <div className="flex flex-col gap-0.5 px-4 border-r border-border/40 last:border-0">
...
export default function CustomerDetails() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const [custData, walletData, txnsData, reqData, compData] = await Promise.all([
  ...
  if (error || !customer) return (
    <div className="flex items-center justify-center min-h-[60vh]">
      <div className="text-center">
        <p className="text-danger font-medium mb-2">{error || 'Customer not found'}</p>
    ...
    <div className="min-h-full bg-background">
      ...
      <div className="px-6 pt-5 pb-0 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1 text-sm text-muted">
          <button onClick={() => navigate('/customers')} className="flex items-center gap-1.5 text-muted hover:text-foreground transition-colors font-medium">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {isBlocked
              ...
              : <><ShieldBan size={15} /> Block Customer</>
            }
          </Button>
          <Button size="sm" onClick={() => navigate(`/customers/edit/${publicId}`)} className="flex items-center gap-1.5">
            Edit Customer
          </Button>
        </div>
      </div>
      ...
      {/* ── Profile Header Card ── */}
      <div className="mx-6 mt-4 bg-surface border border-border rounded-2xl overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center gap-6 px-6 py-5">
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap mb-1">
                ...
                <Badge variant={isBlocked ? 'danger' : 'success'} className="text-xs">
              ...
              <div className="flex flex-col gap-1 mt-1">
                {customer.mobileNumber && (
                  <div className="flex items-center gap-1.5 text-sm text-muted">
                    <Phone size={13} /> {customer.mobileNumber}
                  </div>
                )}
                {customer.email && (
                  <div className="flex items-center gap-1.5 text-sm text-muted">
                ...
                {customer.joinedOn && (
                  <div className="flex items-center gap-1.5 text-sm text-muted">
                    <Calendar size={13} /> Joined on {customer.joinedOn}
                  </div>
                )}
              </div>
            </div>
          ...
          {/* Vertical divider */}
          <div className="hidden lg:block h-20 w-px bg-border/60 mx-2" />
          ...
          <div className="flex flex-wrap items-center gap-0">
            ...
            <StatBox
              label="Verified"
              value={
                <span className={`flex items-center gap-1 ${customer.isVerified ? 'text-success' : 'text-danger'}`}>
                  {customer.isVerified
        ...
        {/* ── Tab Bar ── */}
        <div className="flex border-t border-border/60 overflow-x-auto">
          {tabs.map(tab => {
            const Icon = tab.icon;
            ...
            return (
              ...
              >
                ...
                {tab.key === 'requests'   && requests.length   > 0 && (
                  <span className="ml-1 text-xs bg-primary/15 text-primary rounded-full px-1.5 py-0.5">{requests.length}</span>
                )}
                {tab.key === 'complaints' && complaints.length > 0 && (
                  <span className="ml-1 text-xs bg-danger/15 text-danger rounded-full px-1.5 py-0.5">{complaints.length}</span>
                )}
              </button>
            );
          })}
      ...
      <div className="mx-6 mt-4 mb-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="lg:col-span-2 bg-surface border border-border rounded-2xl overflow-hidden">
              ...
              <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-x-8">
                ...
                <div>
                  ...
                  <InfoRow label="Joined On"      value={customer.joinedOn || customer.created_at} />
            ...
            <div className="bg-surface border border-border rounded-2xl overflow-hidden">
              <div className="flex items-center gap-2 px-5 py-4 border-b border-border/50">
                ...
                <h3 className="font-semibold text-foreground">Wallet Summary</h3>
              </div>
              <div className="p-5 flex flex-col gap-3">
                ...
                <div className="pt-2 border-t border-border/50">
                  ...
                  <p className="text-sm font-bold text-foreground mt-0.5">{transactions.length} transactions</p>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-3 bg-surface border border-border rounded-2xl overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-border/50">
                ...
                {requests.length > 0 && (
                  <button onClick={() => setActiveTab('requests')} className="text-xs text-primary hover:underline flex items-center gap-1">
                ...
                {requests.length > 0 ? (
                  <div className="space-y-2">
                    {requests.slice(0, 5).map(req => (
                      <div key={req.id || req.public_id} className="flex items-center justify-between p-3 rounded-lg border border-border/50 hover:bg-muted/10 transition-colors">
                        <div className="flex items-center gap-3">
                          <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center">
                            <Package size={14} className="text-primary" />
                          </div>
                          <div>
                            <p className="text-sm font-medium text-foreground">{req.service_type || req.type || 'Request'}</p>
                            <p className="text-xs text-muted">{req.public_id || req.id}</p>
                          </div>
                        </div>
                  ...
                  <EmptyState message="No activity yet for this customer" icon={History} />
                )}
              </div>
            </div>
          </div>
        ...
        {activeTab === 'wallet' && (
          <div className="bg-surface border border-border rounded-2xl overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-border/50">
              <Wallet size={17} className="text-primary" />
              <h3 className="font-semibold text-foreground">Wallet & Transactions</h3>
            </div>
            <div className="p-5">
              {/* Balance cards */}
              ...
              {transactions.length > 0 ? (
                <div className="space-y-2">
                  {transactions.map(t => (
                    <div key={t.id} className="flex items-center justify-between p-4 rounded-xl border border-border/50 hover:bg-muted/10 transition-colors">
                      <div className="flex items-center gap-3">
                        <div className={`h-9 w-9 rounded-full flex items-center justify-center ${t.type === 'credit' ? 'bg-success/15' : 'bg-danger/15'}`}>
                          <TrendingUp size={15} className={t.type === 'credit' ? 'text-success' : 'text-danger rotate-180'} />
                        </div>
                        <div>
                          <p className="text-sm font-medium text-foreground">{t.description || 'Transaction'}</p>
                          <p className="text-xs text-muted">{t.created_at ? new Date(t.created_at).toLocaleString() : '—'}</p>
                        </div>
                      </div>
                      <p className={`text-sm font-bold ${t.type === 'credit' ? 'text-success' : 'text-danger'}`}>
                ...
                <EmptyState message="No wallet transactions yet" icon={CreditCard} />
              )}
            </div>
          </div>
        )}
        ...
        {activeTab === 'requests' && (
          <div className="bg-surface border border-border rounded-2xl overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-border/50">
              ...
              <h3 className="font-semibold text-foreground">Requests ({requests.length})</h3>
            </div>
            {requests.length > 0 ? (
              <Table
              ...
              >
                {requests.map(req => (
                  <tr key={req.public_id || req.id} className="border-b border-border last:border-0 hover:bg-muted/5 transition-colors">
                    <td className="px-4 py-3 font-medium text-foreground">{req.public_id || req.id}</td>
                    <td className="px-4 py-3 text-muted">{req.service_type || req.type || '—'}</td>
                    <td className="px-4 py-3 font-medium">₹{req.amount?.toFixed(2) ?? '0.00'}</td>
                    <td className="px-4 py-3"><Badge>{req.status}</Badge></td>
              ...
              <EmptyState message="No requests found for this customer" icon={Package} />
            )}
          </div>
        )}
        ...
        {activeTab === 'complaints' && (
          <div className="bg-surface border border-border rounded-2xl overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-border/50">
              ...
              <h3 className="font-semibold text-foreground">Complaints & Support ({complaints.length})</h3>
            </div>
            {complaints.length > 0 ? (
              <Table
              ...
              >
                {complaints.map(comp => (
                  <tr key={comp.id} className="border-b border-border last:border-0 hover:bg-muted/5 transition-colors">
                    <td className="px-4 py-3 font-medium text-foreground">{comp.complaintId || comp.id}</td>
                    <td className="px-4 py-3 text-muted">{comp.issueType || comp.subject || '—'}</td>
                    <td className="px-4 py-3"><Badge>{comp.status}</Badge></td>
                    <td className="px-4 py-3 text-muted">
                      {comp.date || (comp.created_at ? new Date(comp.created_at).toLocaleDateString() : '—')}
                    </td>
                  </tr>
                ))}
              </Table>
            ) : (
              <EmptyState message="No complaints or support tickets found for this customer" icon={AlertTriangle} />
            )}
          </div>
        )}
      </div>
      ...
      >
        <p className="text-sm text-muted">
          Are you sure you want to {isBlocked ? 'unblock' : 'block'} <strong>{customer.name}</strong>?
          {isBlocked
